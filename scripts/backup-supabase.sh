...
#
# Three rows are backed up: "shared" (every tab's data), "jobs" (the Jobs tab) and "notes" (the
# Notes outliner). The latter two have their own rows so they aren't re-uploaded on every unrelated
# save — see js/jobs.js and js/notes.js. The resulting file is an array of {id, data} objects;
...
HTTP_CODE=$(curl -sS -o "$TMP" -w '%{http_code}' \
  "$SUPABASE_URL/rest/v1/app_data?id=in.(shared,jobs,notes)&select=id,data&order=id.asc" \
...
# The "jobs" and "notes" rows are each optional — absent is legitimate for an account that has never
# used that tab — but if one IS present, null data is an anomaly and fails just like the shared case.
if ! jq -e '
  type == "array"
