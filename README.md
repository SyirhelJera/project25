...
- **A coregame roster can't change** — nobody joins a live match — so ranks, parties and agent stats are fetched **once per match id** and memoized. Steady state is one small presence request every 8–15 seconds. Agent select is the one exception: agents lock in over ~90 seconds, so only those columns are re-read, and when the barrier drops the already-resolved ranks are carried across by puuid rather than fetched again.
