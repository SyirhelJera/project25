...
- New fields on `state` should get defaults added in `persistence.js:applyLoadedState()` so old saved data upgrades in place — this repo does not use migration scripts. **One exception:** `state.jobs` has its own storage resource (a separate `app_data` row `id='jobs'` / `window.storage` key), so new *job-record* fields get their defaults in `jobs.js:applyLoadedJobsState()` instead. Everything else, including `state.jobSiteAccounts`, still belongs in `applyLoadedState()`.
- **Jobs persistence is deliberately split out** (`js/jobs.js` owns `saveJobs()`/`loadJobsData()`): the shared blob is re-uploaded in full on every save from every tab, so a growing application list would be re-sent on every unrelated edit. `loadJobsData()` contains a standing (not run-once) migration that seeds the dedicated resource from a pre-split embedded copy the first time it finds the resource absent — don't "simplify" that away. The main `doSave()` strips `jobs` from its payload via rest-destructuring; a jsonb write replaces the whole column, so anything dropped from that object is destroyed on the next save.
