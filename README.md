...
state = {
  ...
  valorant: { apiKey, accounts:[{id,name,tag,region,platform,current,history:[...],...}], selectedAccountId,
              ...
              live: { enabled, label, regionOverride, historyDepth, showEnemyStats, showIncognito } },
                                       ...
                                       // bundle: name, art, VP price, remainingSeconds from
                                       // checkedAt, and `items` — its contents, each with a base
                                       // and discounted price, taken from the storefront's own
                                       // item list rather than valorant-api's bundle record),
...
**"Log in with browser" — the DevTools trip removed, the ruling above intact.** Sessions expire every 1-3 weeks, and the fix was five manual steps every time, so `scripts/valorant-login-window.mjs` (driven by the tab's **🌐 Log in with browser** button, and by the **🔑 Re-login** button on an expired account's store banner) automates the *copy*, never the *login*. The distinction is the whole design, and it's worth being precise about which half Riot objects to:
...
2. **If deploying outside Claude** (e.g. GitHub Pages), create a Supabase project and:
   ...
   - **For the daily Valorant store check** — this one runs **locally on your own machine**, not as an Edge Function (see "Daily Valorant store check" above for why) — recurring, not one-time. Supports tracking more than one Riot account's store side by side, each saved under a label you choose. No `npm install` needed anywhere in `scripts/` — it's plain Node built-ins end to end:
     ...
     5. **Each saved session expires in roughly 1-3 weeks** (Riot's own limit, not configurable), independently per account. When one does, `valorant-check-store.mjs` writes a "session expired" message into that account's entry in `state.valorant.dailyStores` (shown as a banner under that account's store section on the Valorant tab) instead of failing silently — other accounts keep updating normally. When you see that, the quickest fix with nothing else running is `node scripts/valorant-login-window.mjs <label>` (or double-click `scripts\valorant-login-window.cmd`) — sign in, and that account's saved cookie is replaced in place. With the app open and the local helper running, the **🔑 Re-login** button on that banner does the same thing and re-runs that account's store check as soon as it lands. Repeating steps 1-2 by hand with the same label still works too.
   ...
   - **Optional: trigger Check/Add Account from the Valorant tab itself** instead of a terminal, via `scripts/valorant-local-server.mjs` — a small local bridge (loopback-only, no other network access) that the tab's "Local Helper" panel talks to:
     ...
     5. The token gates `/check`, `/live`, `/login`, `/login-window`, `/login-window-status`, `/login-window-cancel`, and `/delete-account`; regenerate it by deleting `scripts/.valorant-local-token.json` and restarting the server if you ever want to invalidate it.
