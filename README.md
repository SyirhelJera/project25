...
state = {
  ...
  valorant: { apiKey, accounts:[{id,name,tag,region,platform,current,history:[...],...}], selectedAccountId,
              dailyStores: { [label]: {checkedAt,items,bundles,bundle,error} },
              ...
              live: { enabled, label, regionOverride, historyDepth, showEnemyStats, showIncognito } },
                                       ...
                                       // Each entry also carries `bundles` (the week's featured
                                       // bundles — Riot runs one most weeks and two often enough
                                       // that reading a single one dropped the second from the
                                       // app, the widget and the wishlist watch alike. Each has
                                       // uuid, name, art, its own `remainingSeconds` (from
                                       // checkedAt — read from that bundle's own
                                       // DurationRemainingInSeconds, NOT the panel-level
                                       // BundleRemainingDurationInSeconds, which can only ever
                                       // agree with the first bundle), `wholesaleOnly` (Riot's
                                       // WholesaleOnly: true means the contents have no individual
                                       // price and the bundle is the only way to buy any of them —
                                       // surfaced as a "Bundle only" / "Items sold separately"
                                       // chip on the card and a line above the contents list,
                                       // and absent from older checks, where it renders nothing
                                       // rather than guessing), and `items` — its contents, taken
                                       // from the storefront's own item list rather than
                                       // valorant-api's bundle record.
                                       // It is priced twice, like a night-market offer: `price` is
                                       ...
                                       // discountPrice and falls back to the base total. A store
                                       // checked before the list itself existed carries a single
                                       // `bundle` object instead, and every reader still falls
                                       // back to it — valStoreBundles() in valorant.js is the one
                                       // place that resolves either shape — but nothing writes
                                       // both, since the duplicate would re-upload a couple of KB
                                       // of item list per account on every save from every tab),
                                       ...
                                       // Each also carries `subText` (displayNameSubText, the
                                       // offer's type — "CAPSULE") and `description`
                                       // (extraDescription, Riot's own blurb), the two lines the
                                       // in-game featured card shows under the name.
                                       // Riot features the same bundle(s) for everybody, so the
                                       // Shop Tracker renders them once above all accounts, from
                                       // whichever account was checked most recently — as ONE card
                                       // with a tab strip when there are two, see
                                       // valFeaturedBundleHtml() in valorant.js. The card and its
                                       // tabs are keyed by the bundle's own uuid rather than its
                                       // position, so a re-check that reorders the list can't leave
                                       // a lit tab showing a different bundle's contents. The
                                       // countdown is repainted in place by a 1s interval
                                       // (tickValBundleClocks()) and never by a re-render: the
                                       // store card rebuilds wholesale, which would drop hover and
                                       // focus every second and re-run valLearnStorePrices().
                                       // The 10s carousel has the same constraint and so swaps
                                       // only the card's own element (renderValFeaturedBundle()),
                                       // never the store column — a full render every ten seconds
                                       // would put a save() behind a decorative animation. The
                                       // underline's sweep is a CSS animation reading its duration
                                       // from --val-rotate, written from VAL_BUNDLE_ROTATE_MS so
                                       // the bar and the timer can't drift apart; both restart
                                       // together on visibilitychange, since the animation keeps
                                       // running in a hidden tab and the rotation deliberately
                                       // does not. The other three
...
- **`pinterest-feed`** — reads a Pinterest profile's public pins and returns them as JSON. No secret is involved — it exists because half of what it reads (the RSS feeds) serves no CORS headers, so the browser can't fetch those files itself. The Motivation tab uses it for **Pinterest collections**: a category with `source: 'pinterest'` replaces its images with 25 pins the first time the app is opened on a new day (`cat.lastSync` holds the day key), so the slideshow shows something different every morning. Images stay as `i.pinimg.com` URLs — nothing is copied into Storage, so a refresh leaves nothing behind. The 📌 on a thumbnail copies that pin into an ordinary category called **Saved Pins**, which the daily refresh never touches. The username is validated against `^[A-Za-z0-9_][A-Za-z0-9_-]{0,58}$` server-side, and board slugs against `SLUG_RE` — both are interpolated into the fetched URLs' paths, so those checks are the whole SSRF guard.

  Request shape is `{ username, boards?, creators?, discover?, keywords?, exclude?, hits? }` → `{ pins, boards, boardSlugs, creatorIds, discovered?, matched?, hits? }`.

  **Two sources, pidgets first.** `api.pinterest.com/v3/pidgets/{users/<user>,boards/<user>/<slug>}/pins/` is the public JSON behind Pinterest's embeddable widgets: no key, no cookie, 50 pins a call against RSS's 24–26, and — the part that matters — **each pin names the board it lives on**, which is now the only working way to discover boards at all. RSS (`/<user>/feed.rss`, `/<user>/<board>.rss`) is kept as the fallback for any board pidgets refuses and for the profile as a whole, so one endpoint changing shape can't empty a collection. Both are normalised into the same `{id, url, fallbackUrl, link, title}` record — note a pidgets pin's own `link` is the *destination website*, not the pin page, so the pin page URL is built from its `id`, and it is that URL the `resolve` action is keyed by.

  **Video pins.** The same function has a second action, `{ resolve: [pinUrl, …] } → { videos: { [pinUrl]: mp4Url } }`, which is how a collection plays video pins rather than showing their cover frame as a still. It's separate from the feed read because neither source says whether a pin is a video — finding that out means fetching the pin *page* and pulling the mp4 out of the JSON embedded in it (unescape `\/`, then match `v*.pinimg.com/videos/….mp4`, preferring the 720p rendition; `.m3u8`/HLS variants are excluded since `<video>` can't play them outside Safari). Doing that for the whole merged pool of 138–474 pins would dwarf the merge itself, for pins about to be discarded — so the client picks its 25 first and asks about only those, through the same 6-wide worker pool. Client-side (`resolvePinterestVideos()` in `js/motivation.js`) the call is deliberately **fire-and-forget after the images are already on screen and saved**: nothing about the collection depends on it, so a slow or failed resolve just leaves the pins as stills, exactly as they behaved before this existed. Results are patched back **by record id, never by index**, since another sync or a delete may have rewritten the list mid-flight. Pin URLs arriving from the client are validated against `PIN_URL_RE` server-side — same role as the username check, and the whole SSRF guard for this action.
  ...
  **Why it fetches every board, not just the profile.** The profile feed is a fixed window of the ~25 most recent saves with no pagination (`?page=` and `?limit=` are ignored, verified), and the pidgets user endpoint is a similar 50-item window — so on their own the collection could only ever show what you pinned lately, and a slideshow drawing 25 a day out of a 24-pin pool is *the same photos every morning*. Each **board** has its own window, and a board you last added to a year ago returns year-old pins, so merging every board is what reaches back into the archive: measured **474 unique pins across 9 boards** on a real profile, versus 24 from the profile feed alone. Boards are fetched through a 6-wide worker pool (a profile with 30 boards would otherwise open 31 sockets at once, which is where Pinterest starts rate-limiting), and the merged pool is capped at `MAX_PINS` = 800 — sized so a normal profile comes back *whole*, because the client shows each pin once before repeating any and a pool truncated mid-fan-out would silently hide whichever boards lost the race to fill it. Nothing of it is stored; the client keeps 25.

  **Board discovery, and why it needed rebuilding.** This used to scrape board links out of the profile page's HTML. Pinterest has since turned off server-rendering for logged-out visitors — its own embedded page data lists `lop_unauth_lockdown_profile` / `_board` / `_search` as enabled — so that scrape now returns **nothing at all, on every profile**, and the merge had silently collapsed to the ~24-pin profile feed. That is what made a “fresh every day” collection show an identical slideshow every morning. Discovery is now three sources, cheapest and most reliable first:

  1. **What the client already knows.** `cat.boardSlugs` rides along in the request and is written back from the response, so the known set only ever *grows*. This is the load-bearing one: the other two only see boards you pinned to recently, and without a remembered list the pool would shrink back to “recent” on every sync.
  2. **The `board` object on each pidgets pin.** Authoritative, free, already fetched — but it only covers boards touched within your last 50 saves. The board URL's `/<user>/` half is matched back against the username, which is what stops a collaborative board on someone *else's* profile joining this profile's fan-out.
  3. **A plain web search** (`site:pinterest.com/<user>/` via DuckDuckGo's HTML endpoint), best-effort, and only when the first two came up thin (`SEARCH_DISCOVERY_BELOW` = 5 known boards). It is the only source that can name a board you haven't pinned to in months — which is exactly the archive the slideshow wants — but it is someone else's HTML, it may be rate-limited from a datacenter IP, and a private profile isn't indexed at all. Every one of those degrades to `[]` and the merge carries on with the boards it already had. Once a profile's boards are known and remembered, this call never happens again.

  The response's `boardSlugs` lists only the boards that actually **returned pins**; the client unions it with what it had, working ones first, so a board gone quiet drifts to the tail of its 30-slug list and eventually falls off, while a one-off read failure costs nothing. Note this reads the profile's *own* pins throughout: the logged-in home feed (pins from accounts you follow) is private, with no RSS or public API — and Pinterest's internal `/resource/…Resource/get/` endpoints, which would serve related pins and search, answer `Invalid Resource Request` without a session, so there is no keyless route to photos you never pinned.

  **Why the same photos stopped recycling: turn-taking, not random draw.** `pickPinterestPins()` in `js/motivation.js` used to shuffle the pool and take the first 25. That samples the archive *with replacement* across days — with 300 pins and 25 a day you meet about two of yesterday's every morning and never work through the collection. It now prefers pins the collection hasn't shown yet: every pin in the pool gets a turn before any gets a second, so a 300-pin pool is twelve days of genuinely new photos and a 474-pin pool nineteen. When the unseen set runs dry the cycle restarts, topped up from the already-shown **in the order they were shown**, so the pins seen longest ago come back first and today still can't repeat yesterday. Turns live in `cat.seenIds`, capped at `PINTEREST_SEEN_CAP` = 600 — which also makes it a sliding window on a pool larger than the cap, rather than a hard cycle. Entries are the pin's **numeric id**, not its page URL: this list rides the shared blob, re-serialized and re-uploaded in full on every save from every tab, and at 600 entries a 19-character id against a 49-character URL is ~20KB off every one of those saves (measured 12.9KB at the cap). Both `cat.seenIds` and `cat.boardSlugs` are cleared when the category's username changes — the slugs belong to the old profile and would be fetched under the new one as a fan-out of 404s, and the turn history describes pins that aren't in the new pool. The once-a-day sync logs `picked N of M pins across B boards (F never shown before)` to the console; `F` dropping below 25 means the pool has been worked through and a new cycle started, not that something is broken.

  **`pinimg.com` has to stay in `sw.js`'s `LIVE_DATA_HOSTS`, and it is turn-taking that put it there.** The service worker's cross-origin branch is cache-first into a `RUNTIME_CACHE` that has no eviction and no size cap. While a collection recycled one set of 25 photos that was ~25 cached images and nobody noticed; showing 25 *different* ones a day makes it 25 new images a day forever — hundreds of megabytes on a phone within months, for photos replaced tomorrow. Listing the host leaves stills and clips to the browser's own HTTP cache, which is size-managed and evicted; repeat views within a day cost the same, and the only thing given up is an offline slideshow of images that were remote to begin with. `RUNTIME_CACHE` was bumped to `v2` in the same change, since the `activate` handler deletes every cache not in `CURRENT_CACHES` and that rename is what reclaims whatever an earlier build had already written.

  **Discover — pins you have never saved.** Turn-taking fixes repetition, but a collection drawn from your own boards can still only ever show you things you already chose; the point of a morning slideshow is new ideas. There is no keyless route to “pins like these” — verified four ways: Pinterest's internal `/resource/RelatedPinFeedResource/get/` and `BaseSearchResource` answer `Invalid Resource Request` (403) without a session, its `/search/pins/` and `/topics/` pages server-render no pins to a logged-out visitor, there is no search or topic RSS, and pidgets has no related, search or `pins/info` method (403/404). What pidgets *does* carry on every pin is **`native_creator`** — the id of the Pinterest user who made the pin you saved — and the `/v3/pidgets/users/<x>/pins/` path **accepts a numeric user id wherever a username goes**. That equivalence is the whole feature: the taste graph is already sitting in your own boards, in the form of the people you save *from*.

  So `{ username, discover: true, creators }` harvests `native_creator` ids out of your own pins, samples `DISCOVER_CREATOR_SAMPLE` = 12 of them at random, reads their recent pins, and subtracts every pin id the profile already has. Measured on a real profile: **131 creators behind 149 saved pins, 121 readable, 5,805 pins never saved** — and it grows every time you pin something. Driven for ten consecutive days through the real picker it returned **250 distinct pins across 250 slots: zero repeats, zero already-saved leaks**, ~2s and ~176KB a sync.

  The rules it rests on. The creator sample is **random every sync and deliberately small**: reading all 131 would push a daily sync past half a minute for pins thrown away unseen, and re-reading the *same* twelve would rebuild the same candidate pool — which is the exact complaint the feature exists to answer. Drawing a fresh twelve makes the reachable pool the whole creator list. `cat.creatorIds` is remembered client-side and sent back, the same growing-set role `boardSlugs` plays, because a sync only harvests creators from the pins it happened to read. A creator that can't be read is **skipped, never fatal** (10 of the 131 were gone or private) — a slideshow that breaks because one account went private is a broken slideshow. Exclusion is **by pin id against the profile's own pins gathered in the same request, never against `cat.seenIds`**: that list does a different job and is capped, so using it here would let saved pins back in the moment it rolled over. This is also why the RSS path carries a `pinId` it otherwise has no use for — a board read through RSS rather than pidgets must still contribute to the exclusion set. And `pinId`/`creator` are **stripped at the wire boundary** (`forWire()`): the client reads neither, they are ~20KB of a 500-pin response, and dropping them keeps the record the client sees to one shape whatever source it came from.

  A failed or empty discover pass **falls back to your own pins rather than erroring**, and says so (`discovered: 0`) — discover is a preference, and a collection that empties itself because a first sync hasn't harvested anyone yet is worse than one showing pins you have seen. It is a per-collection toggle (✨ **Discover new pins** in the collection menu, `cat.discover`) that **defaults on**, stored as “not false” so a collection predating the key starts showing new pins without being switched on; flipping it clears `lastSync` and re-syncs immediately, since a toggle whose effect only appears tomorrow morning reads as a toggle that did nothing. **Saved Pins** is created with `discover: false` — it is an ordinary category the refresh never touches. Changing the username clears `creatorIds` along with `boardSlugs` and `seenIds`: they are the other account's taste graph.

  **Keywords — saying what the collection is for.** Discover reads a creator's *whole* recent feed, which is indiscriminate by construction: someone who made one car photo you saved also pins recipes, nail art and — measured on a real profile — outright scam boards (“Cashapp flip”, “Bank log”). 🔎 **Keywords…** in the collection menu takes one comma-separated line, with a leading minus marking a word to block: `cars, aviation, -makeup`. Stored as `cat.keywords` / `cat.excludeWords`, two arrays — the minus is a detail of that editor and never reaches the request. Editing them clears `cat.hitCreators` and re-syncs at once, since the pins on screen were chosen under the old terms.

  Matching is **whole-word with plural tolerance, never prefix**. This was measured, not assumed: prefix matching had “car” hitting “hair CARE”, “CARPET” and “CARDIGAN” — the same off-topic noise the filter exists to remove, arriving *through* the filter. So a term matches a word, or that word ± a trailing `s`/`es` (compared both directions, since stemming disagrees with itself across “dress”/“dresses”), and a multi-word term matches only as a consecutive phrase. The cost is that “car” misses “supercar”; with thousands of candidates and 25 slots a day, precision is worth far more than recall and a second keyword covers the gap.

  **The two text fields are not equal evidence**, which is the other measured finding. A pin's `boardName` is the board its creator filed it under and is the honest topic — a board called “cars 🏎️” holds car photos. A `desc` is marketing copy: the description-only matches for car terms were a *running* pin whose blurb said “blacked out Porsche” and a *t-shirt* pin listing “Car Tshirts • BMW”. So an **include** term is tried against the board name first, and description matches are added only when board matches alone can't fill a day (`DISCOVER_MIN_POOL` = 60); an **exclude** term is matched against both at once, because not wanting nail art is a reason to drop a pin however the words got there. Both fields are internal — `forWire()` strips them alongside `pinId`/`creator`, so the filter costs the client nothing and the record it sees stays one shape.

  Two things make a narrow topic viable. The pidgets **board** endpoint omits the `board` object that the *user* endpoint puts on every pin (verified), so `fetchPidgetsBoard()` passes the slug in as the name — without it the primary field is empty for every pin on your own boards, which is most of them, and “car” matched 1 of your own pins instead of 10. And the creator sample is **steered rather than random once a filter is on**: creators are read preferentially if they matched before (`cat.hitCreators`, returned as `hits` and merged rather than replaced) *or* — with no history at all — if their pins were ones you filed on a board of your own that the keywords describe, which is what makes the very first filtered sync land on topic. Two thirds of each round comes from that preferred set and a third stays fresh, so the pool sharpens without ossifying, and `DISCOVER_MAX_ROUNDS` = 3 batches are drawn while the on-topic pool is still thin — bounded by rounds, never by “until we have enough”, or a keyword nobody in your graph pins about would walk all 131 creators in one sync. Measured on a real profile, those two changes took “gym, workout, fitness” from 19 pins to 73 and “car, jdm, porsche” from 1 to 16.

  A filter that matches nothing **falls back to the unfiltered pool and reports `matched: 0`** rather than emptying the collection, and the client says which fallback it hit — `matched: 0` (“your keywords matched nothing”) and `discovered: 0` (“couldn't find anything new”) are different sentences, and only ever shown on a sync you asked for. **What a keyword can reach is bounded by who you save from**, and that limit is real: on the test profile “aviation” returns almost nothing, because the creators behind those saved pins are German travel bloggers who pin hotels and islands rather than aircraft — “travel” reaches them, “aviation” does not. Terms are capped at `MAX_TERMS` = 20 per list and `MAX_TERM_LEN` = 40 chars, validated on both sides: they ride the shared blob and are matched against every pin in a pool of up to 800.

### Pinterest home feed (local helper)

Everything above reconstructs a feed out of public data. The **real** home feed — the one at pinterest.com when you're signed in, that Pinterest personalises for you — has no public route at all, and this is the only thing in the app that reads it.

**Why it can't be an Edge Function.** Pinterest's official API has **189 endpoints and not one returns a feed** (checked against [Pinterest's own OpenAPI spec](https://github.com/pinterest/api-description)): `/search/pins` searches only your own pins, and there is no recommendations, related-pins or home-feed resource. The pidgets JSON everything above uses is public precisely because it serves *embeddable widgets*, and a personalised feed is not embeddable. Its internal `/resource/UserHomefeedResource/get/` is what the website's own front end calls, and it answers only to a logged-in session. That cookie is **full account access** — it can pin, unpin, follow, message, change the account — and this app's Supabase row is unauthenticated, so it can never go into `state`, a Supabase row, or a cloud function. It lives in a gitignored `scripts/.pinterest-session.json` on the machine you signed in on, read by a process running as you: the same ruling, file layout and reasoning as the Riot session in `scripts/valorant-lib.mjs`.

**The pieces.** `scripts/pinterest-lib.mjs` owns the session file and the feed read; `POST /pinterest-feed` on the existing local helper serves it (one port, one token, one “helper not running” story for a bridge you already run — it touches no Valorant file); `fetchPinterestHomeFeed()` in `js/motivation.js` calls it. Two ways in, exactly as the Valorant login has: `node scripts/pinterest-login.mjs` pastes the cookies, and `scripts/pinterest-login-window.mjs` opens a fresh browser window at Pinterest's login page and lifts the jar out of it via CDP `Storage.getCookies` on the **browser** target. The same red line applies there: **no anti-detection flag may ever be added to that argv**, the read never attaches to the page, and the throwaway profile is wiped on every terminal state because it holds a live login until it is. 🏠 **Home feed sign-in…** in the collection menu drives both, and the menu entry hides itself where no helper could answer.

**The rules that hold it up.** It is **preferred, never a mode you pick**: when the helper answers it *is* the pool, because it is the feed you already like rather than one reconstructed from the creators behind your saves — and it returns `null` on a phone, with the helper stopped, or before you have signed in, which is what makes the public Discover path a fallback rather than dead code. It runs **only under Discover**, since with Discover off you asked for your own saves and this isn't them. Pins are normalised into the **same `{id, url, fallbackUrl, link, title}` record** the Edge Function emits, so the picker, the turn-taking, the video resolve and Saved Pins can't tell the two sources apart — `commitPinterestPicks()`/`finishPinterestSync()` are shared by both paths for that reason. **Promoted pins are dropped**: the home feed carries adverts inline and a slideshow you look at every morning is the last place they belong. The feed is **bookmark-paginated**, the only source here that isn't a fixed window, so `MAX_PAGES`/`PAGE_SIZE` bound how far one refresh walks rather than a lack of data doing it. Keyword matching is **duplicated** in `pinterest-lib.mjs` rather than shared with the Edge Function — one runs in Node, the other in Deno, and this repo has no bundler; if the matching rules change they change in both places. Every request it makes is a **GET against the feed**: nothing here writes to Pinterest. And failures are **states, not faults** — `no_session` and `expired` come back as `200 {ok:false, code}` (the `/tft-live` shape), the collection quietly falls back, and only a sync you asked for says anything out loud.

**Signing in: verified before it is saved, and `_auth` is the test.** `_pinterest_sess` and `csrftoken` are set for signed-OUT visitors too — they are the *session*, not the *login* — so waiting on their presence captures a logged-out jar the instant the login page loads. `_auth` is the flag that actually distinguishes them (`"1"` signed in, `"0"` not), and the browser-window loop gates on it. The sharper rule is the ordering: **nothing is written until a real feed read has succeeded** (`verifySession()`), on both the window and the paste paths. The first version saved first and verified after, so a failed verify left a signed-out jar in the session file — which the app then reported as “expired” on every refresh while quietly falling back to the public path, i.e. exactly like the feature working badly rather than not running at all. A `_auth=0` jar gets its own message, separate from “expired”, because it means the sign-in did not finish rather than that it lapsed; a jar with no `_auth` at all (the paste path can only see the two cookies in DevTools) falls through to the real read, which is the proof either way. Let the sign-in window **close itself** — closing it by hand a moment early is how a half-finished jar gets picked up.

**And the fallback is never silent.** A manual refresh that wanted the home feed and could not read it says so and names the reason, and the 🏠 menu entry carries the last verdict (`not signed in` / `sign-in expired` / `on`). A helper that simply isn't reachable — a phone, or the helper stopped — is deliberately NOT reported as a sign-in problem: no note, neutral label, quiet fallback, because there nothing is wrong.

**The accepted cost:** the refresh needs the helper running on your PC. Your phone still *sees* that day's pins — only the 25 picks are stored in `state` — but a refresh there falls back to the public path. That is the same trade every local-helper feature in this repo makes, and the reason the public path is maintained rather than retired.
...
| Pinterest pidgets (`api.pinterest.com/v3/pidgets/{users,boards}/…/pins/`) + RSS (`pinterest.com/<user>/feed.rss` + each `/<user>/<board>.rss`), via the `pinterest-feed` function | The pin pool a Motivation "Pinterest collection" draws its daily 25 from | None (public widget JSON + public profile/board feeds) |
| DuckDuckGo HTML (`html.duckduckgo.com/html/?q=site:pinterest.com/<user>/`, via the `pinterest-feed` function) | Best-effort board discovery, only when fewer than 5 boards are already known — Pinterest no longer server-renders its own board lists to logged-out visitors | None (public search) |
| Pinterest pidgets, again, keyed by the `native_creator` id on your own pins | “Discover” — the pins a collection shows that you have never saved | None (public widget JSON) |
| `pinterest.com/resource/UserHomefeedResource/get/`, **via the local helper only** | Your real, personalised Pinterest home feed — the best pool a collection can show | Your own Pinterest session cookie, in gitignored `scripts/.pinterest-session.json`, never uploaded |
...
supabase/functions/
  ...
  pinterest-feed/                   merges a Pinterest profile's public pins — pidgets JSON first, RSS as fallback, profile + every board (for Motivation's Pinterest collections); also discovers board slugs and resolves mp4 URLs for video pins
...
scripts/pinterest-lib.mjs           shared, dependency-free helpers for the Pinterest home feed (session file, the feed read, keyword matching)
scripts/pinterest-login.mjs         run LOCALLY to save your Pinterest cookies by pasting them (--check tests the saved one, --forget deletes it)
scripts/pinterest-login-window.mjs  run LOCALLY, standalone — opens a small signed-out window on Pinterest's login page, waits for you to sign in, and saves the resulting cookie; also powers the collection menu's "Home feed sign-in" button
