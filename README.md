...
- **`pinterest-feed`** — reads a Pinterest profile's public RSS feeds and returns its pins as JSON. No secret is involved — it exists purely because Pinterest serves no CORS headers, so the browser can't read those files itself. The Motivation tab uses it for **Pinterest collections**: a category with `source: 'pinterest'` replaces its images with 10 random pins the first time the app is opened on a new day (`cat.lastSync` holds the day key), so the slideshow shows something different every morning. Images stay as `i.pinimg.com` URLs — nothing is copied into Storage, so a refresh leaves nothing behind. The 📌 on a thumbnail copies that pin into an ordinary category called **Saved Pins**, which the daily refresh never touches. The username is validated against `^[A-Za-z0-9_][A-Za-z0-9_-]{0,58}$` server-side — it's interpolated into the fetched URLs' paths, so that check is the whole SSRF guard.
...
| Pinterest RSS (`pinterest.com/<user>/feed.rss` + each `/<user>/<board>.rss`, via the `pinterest-feed` function) | The daily 10 random pins in a Motivation "Pinterest collection" | None (public profile + board feeds) |
...
supabase/functions/
  ...
  pinterest-feed/                   merges a Pinterest profile's public RSS feeds — profile + every board (CORS proxy for Motivation's Pinterest collections)
