...
`js/realtime.js` makes a change on one device show up on the others in about a second, no reload. It covers the **shared row only** — Jobs and Notes keep their own rows and their existing save-and-conflict-banner model. Requires `supabase/setup-realtime.sql` to have been run once (it adds `app_data` to the `supabase_realtime` publication); without it the file degrades to a single console warning and the app behaves exactly as it did before.

Before this existed, `load()` ran once at boot and nothing re-read the row. The cost wasn't just staleness: the stale device's `lastKnownUpdatedAt` also failed the optimistic-concurrency check on its *next* save, so it was blocked from writing until you reloaded it.

- **Transport: Postgres Changes, not client broadcast.** The Valorant store scripts write to this row from outside any browser (`valorant_set_daily_store`, see "Daily Valorant store check" below), and a browser-to-browser broadcast would never see them. The subscription is filtered to `id=eq.shared`, so a Notes keystroke on another device doesn't push the notes row to clients that don't sync it.
- **Three-way merge at top-level-key granularity.** Replacing state wholesale would discard whatever the receiving device had edited but not yet saved. Instead each top-level key of `state` is compared against a snapshot of both sides as of the last time they agreed: only remote moved → take theirs; only we moved → keep ours and push it; both moved → take theirs (that's the value that actually committed) and offer a one-click undo. Because the tabs are independent, a Finance edit here and a Goals edit there simply both survive, and the conflict banner now only appears for a genuine same-tab collision.
- **Two base snapshots, not one.** `applyLoadedState()` fills in field defaults *in place*, so the hydrated local object is legitimately not byte-equal to the raw payload the row holds. `syncBaseLocal` tracks the hydrated view and `syncBaseRemote` the raw one, and each side is only ever compared against its own kind — with a single base, every key would read as permanently changed. For the same reason `captureRemoteBase()` must run *before* `applyLoadedState()` gets at the payload.
- **jsonb doesn't preserve key order**, so all comparisons go through `stableStringify()` (recursive key sort). Plain `JSON.stringify` would report a difference on every key on the first event after load.
- **Echo suppression**: `doSave()` mints `updated_at` itself and only assigns `lastKnownUpdatedAt` once the round trip returns, so the Realtime event for our own write regularly arrives first. Stamps are registered via `noteOwnWrite()` before the request and compared by instant rather than by string (Postgres' wire format for a timestamptz isn't byte-identical to the ISO string PostgREST echoes back).
- **Targeted re-render.** A changed key maps to just the renderers it affects, rather than `renderAll()` — which rebuilds all twelve tabs and resets the goals working-carousel's scroll position. An unmapped key falls back to `renderAll()`, so a top-level `state` key added later can't silently go stale.
- **The merge always applies immediately; only the re-render waits.** Every `render*()` here rebuilds its section's `innerHTML` and would throw away the caret, so a queued render holds until `focusout` (with a 5s ceiling). Data and timestamps still update instantly, which is what keeps the next local save from conflicting for no reason.
- **Catch-up fetch** on subscribe, tab-visible, window-focus and `online`. Realtime only delivers what happens while the socket is up, and a phone that went in your pocket won't report that its socket died. Two-step on purpose — it probes `updated_at` alone and only pulls `data` if it moved, so alt-tabbing doesn't drag the whole blob down each time; it's also throttled to one probe per 5s. The same path handles Realtime dropping an oversized record (past `max_record_bytes`, ~1MB, it sends `payload.errors` with no `new`), so a growing blob degrades to a re-read rather than to silence.
- **Ordering**: applying a remote change is chained onto `persistence.js`'s `savePromise`, the same queue `doSave()` uses, so a merge can't land mid-save and move `lastKnownUpdatedAt` out from under it.
- **Derived state** (`recomputeDailyActivity()`, `snapshotNetWorth()`) is normalized before the comparison and re-derived after it, and the base is captured *after* that re-derivation. Both are pure functions of `checklists`/`finance` keyed by today's date, so the devices converge — and capturing the base last is what stops a derived-only difference from triggering a push of its own, i.e. what stops two devices trading saves forever.

Realtime inherits the same "no login, anyone with the link" trade-off as the rest of the app: it authorizes subscribers against `app_data`'s existing permissive RLS policy, so treat the live feed as no more private than the row itself.
...
2. **If deploying outside Claude** (e.g. GitHub Pages), create a Supabase project and:
   ...
   - **For live cross-device sync** — run `supabase/setup-realtime.sql` once in the SQL editor. It adds `app_data` to the `supabase_realtime` publication, which is what lets `js/realtime.js` receive row changes (see "Live cross-device sync" above). Skipping it doesn't break anything: the app just goes back to loading once per page open, and logs one console warning. Verify under Database → Replication that `app_data` is listed.
...
js/
  ...
  realtime.js                       live cross-device sync of the shared row (Supabase Realtime + per-key merge)
...
supabase/setup-realtime.sql         run once — publishes app_data over Realtime, enabling js/realtime.js
