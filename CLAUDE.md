...
- `index.html` loads a **fixed sequence** of `<script>` tags from `js/` — order matters, later files reference functions/globals defined in earlier ones:
  ...
  countdowns.js → insights.js → backups.js → mantras.js → motivation.js → checklists.js →
  notes.js → finance.js → wishlist.js → jobs.js → fitness.js → valorant.js → clock.js →
  realtime.js → main.js
  ```
  `realtime.js` sits second-to-last because it dispatches to every `render*()`; `main.js` stays last because it's what calls `load()`. Anything added to this list also needs adding to `sw.js`'s `SHELL_ASSETS` (and a `SHELL_CACHE` version bump) — `cache.addAll()` is all-or-nothing, so a name that doesn't resolve fails the whole precache.
...
- **Live cross-device sync** (`js/realtime.js`, loaded second-to-last): a Supabase Realtime `postgres_changes` subscription on the **shared row only**, feeding a three-way merge at top-level-`state`-key granularity. Jobs and Notes are not synced. Needs `supabase/setup-realtime.sql` run once; without it the file no-ops after one console warning. Read README's "Live cross-device sync" section before touching it — three things are load-bearing and none are obvious from the code alone: comparisons go through `stableStringify()` because **jsonb doesn't preserve key order**; there are **two** base snapshots (`syncBaseLocal` hydrated / `syncBaseRemote` raw) because `applyLoadedState()` adds defaults *in place*, so `captureRemoteBase()` must run **before** it; and the base is captured **after** `recomputeDailyActivity()`/`snapshotNetWorth()` re-run, which is what stops two devices trading saves forever.
...
- Adding a whole new top-level `state` key needs no change to `doSave()` — the rest-destructure carries it automatically. It *should* get a `SYNC_RENDERERS` entry in `realtime.js` though: without one it still syncs correctly, but a remote change to it falls back to a full `renderAll()`.
...
- Merge or apply remote state anywhere except `realtime.js:mergeRemote()`. It's the only place that knows the raw-vs-hydrated snapshot distinction; a hand-rolled `applyLoadedState(remote)` elsewhere silently discards whatever this device edited but hadn't saved yet.
- Call `captureRemoteBase()` *after* `applyLoadedState()`, or drop the `captureSyncBase()` call from `doSave()`'s success path. Either one leaves the base describing the wrong thing, and every key then reads as permanently changed — which shows up as an endless save loop between devices, not as an error.
...
- `supabase/setup-realtime.sql` → run once by hand in the SQL editor, like the other `setup-*.sql` files. Live sync is inert until then, with no in-app symptom beyond a console warning.
